...
def draw_repeating_background(background_img):
    ...
    for i in range(int(math.ceil(WINDOW_WIDTH / background_rect.height))):
...
def game_loop():
    ...
    while True:
        ...
        if paused:
            ...
            occur_of_rocks = 1 + int(score / 50)
            min_rock_speed = 1 + int(score / 40)
            max_rock_speed = 1 + int(score / 30)
            ...
            if random.randint(1, occur_prob) == 1:
                ...
                if random.randint(1, occur_prob * 10) == 1:
                    warp = Warp(random.randint(30, WINDOW_WIDTH - 30), random.randint(30, WINDOW_HEIGHT - 30))
                    warps.add(warp)
        ...
        warps.update()
        rocks.draw(screen)
        warps.draw(screen) #fdjskfld;sjfkdls;;fjdksla;sdhajkhdm,mh
...
def game_screen():
    ...
    draw_text('준서의 우주 암석 피하기', pygame.font.Font('NanumGothic.ttf', 70), screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 3.4, WHITE)
    draw_text('점수: {}'.format(score), default_font, screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2.4, YELLOW)#{ }
    draw_text("마우스 버튼이나 's'키를 누르면 게임이 시작됩니다", default_font, screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2.0, WHITE)
    draw_text('게임을 종료하실려면 "Q"키를 누르세요', default_font, screen, WINDOW_WIDTH / 2, WINDOW_HEIGHT / 1.8, WHITE)
